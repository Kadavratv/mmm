...
async def a(client, message: Message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = await message.reply(f"**{Bn} :-** 👨‍💻 Arama Yapılıyor {query}")
    ...
    try:
        ...
        try:
            ...
            m.edit(f"**{Bn} :-** 🙇‍♂️ hiçbir şey bulamadım. Aradığınız Şeyi Biraz Değiştirin.\n\n{e}")
            return
    except Exception as e:
        m.edit(
           f"**{Bn} :-** 🙇‍♂️ Hiçbir şey bulamadım. Üzgünüm.\n\nTry Başka Bir Aramayı Veya Yazıyı Biraz Değiştirin !Bulacaksın! "
        )
        print(str(e))
        return
